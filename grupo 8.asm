...
; CONSTANTES
; --------------------------------------------------
BUFFER	EQU	4000H         ; guarda a tecla
PIN     EQU 0E000H       ; entrada do teclado
POUT    EQU 0C000H       ; saida do teclado
pixelsMatriz EQU 8000H   ; inicio do ecrã
...
; --------------------------------------------------
PLACE 3250H
segundo_atual:    WORD 0    
contador_decimal: WORD 0    
...
fantasma_linha:  WORD 14      
fantasma_coluna: WORD 14
fantasma_dir:    WORD 3       

; Posições do Fantasma 2
fantasma2_linha:  WORD 14     
fantasma2_coluna: WORD 14
fantasma2_ativa:  WORD 0     
fantasma2_timer:  WORD 0     
...
inicio:
    MOV SP, stackBase     ; Inicia o registro da pliha
    CALL Carregamento     

; Principal ciclo do jogo 
main_loop:
    CALL pTeclado         
...
jogo_ativo:
    ...
    MOV R3, R2
    CMP R2, 1H            ; Tecla 1 (direcionar para cima)
    JZ mover_cima

    MOV R4, 9H
    CMP R2, R4            ; Tecla 9 (direcionar para baixo)
    JZ mover_baixo
    CMP R2, 4H            ; Tecla 4 (direcionar para esquerda)
    JZ mover_esquerda
    CMP R2, 6H            ; Tecla 6 (direcionar para direita )
    JZ mover_direita
    MOV R4, 0FH
    CMP R2, R4           ; Tecla F (sair do jogo)
...
continuar_jogo:
    ...
    ; Verificação do jogo para ver se ainda esta ativo
    MOV R1, game_active
    MOV R1, [R1]
    CMP R1, 0
    JZ main_loop
    ...
    CALL mover_fantasma2_lento    
    CALL verificar_colisao
    CALL verificar_colisao2       
    CALL verificar_colisao_objetos
    
    ; Verificar se o objectivo foi concluido
    CALL verificar_vitoria_global
    
    ; Se o objetivo foi concluido pula o resto
    MOV R1, game_active
    MOV R1, [R1]
    CMP R1, 0
    JZ main_loop
...
ativar_segundo_fantasma:
    ...
    ; Verificar se passaram segundos
...
mover_fantasma2:
    ...
    ; posição atual do fantasma 2
    MOV R1, fantasma2_linha
    MOV R5, [R1]        ; R5 = linha do fantasma 2
    MOV R2, fantasma2_coluna
    MOV R6, [R2]        ; R6 = coluna do fantasma 2
    
    ; Calcular diferenças entre o pac man e os fantasmas
    MOV R1, R3
    SUB R1, R5          ; Difere a linha (Pac-Man - Fantasma2)
    
    MOV R2, R4
    SUB R2, R6          ; Difere a coluna (Pac-Man - Fantasma2)
...
; mover o fantasma de forma lenta
;------------------------------------------------
...
; mexer o pac man para mover_baixo
; --------------------------------------------------
mover_pac_baixo:
    PUSH R1
    PUSH R2
...
; mover fantasma para perseguir pac man
; --------------------------------------------------
mover_fantasma:
    PUSH R1
    PUSH R2
